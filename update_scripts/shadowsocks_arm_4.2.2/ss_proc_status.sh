...
echo_version(){
	...
	if [ -z "$ss_basic_v2ray_version" ];then
		ss_basic_v2ray_version_tmp=`/koolshare/bin/v2ray $V2RAY_VERSION 2>/dev/null | head -n 1 | cut -d " " -f2`
	...
	if [ -z "$ss_basic_v2ray_date" ];then
		ss_basic_v2ray_date_tmp=`/koolshare/bin/v2ray $V2RAY_VERSION 2>/dev/null | head -n 1 | cut -d " " -f5`
